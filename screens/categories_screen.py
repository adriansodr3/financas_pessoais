...
class CategoriesScreen(MDScreen):
    ...
    def refresh(self):
        ...
        for cat in CategoryModel.get_all(uid):
            ...
            ico = IconLeftWidget(icon="arrow-up-circle-outline" if is_inc else "arrow-down-circle-outline",
    ...
    def _on_item(self, cat):
        ...
        def do_del():
            ...
            self.refresh()
        confirm_dialog("Excluir categoria",
            "{} {}\nLancamentos existentes nao serao afetados.".format(cat.get("icon",""), cat["name"]),
    ...
    def add_cat(self):
        ...
        content = MDBoxLayout(orientation="vertical", spacing=dp(8),
                              size_hint_y=None, height=dp(200), padding=[0, dp(8), 0, 0])
        f_name = MDTextField(hint_text="Nome", mode="rectangle", size_hint_y=None, height=dp(52))
        f_icon = MDTextField(hint_text="Icone emoji (ex: 🏠)", mode="rectangle", size_hint_y=None, height=dp(52))
        ...
        def open_type(btn):
            ...
            items = [{"text": nm, "viewclass": "OneLineListItem",
                      "on_release": lambda t=tp, n=nm: (
                          setattr(self,"_sel_type",t), btn.__setattr__("text",n),
                          btn.__setattr__("md_bg_color", RED if t=="expense" else GREEN),
                          menu.dismiss())} for tp,nm in [("expense","Despesa"),("income","Entrada")]]
            menu = MDDropdownMenu(caller=btn, items=items, width_mult=3)
            menu.open()
        btn_type.bind(on_release=open_type)
        for w in [f_name, f_icon, btn_type]: content.add_widget(w)
        dlg = [None]
        def save(*a):
            if not f_name.text.strip(): return
            CategoryModel.create(uid, f_name.text.strip(), self._sel_type, "#6366f1", f_icon.text.strip() or "?")
            dlg[0].dismiss(); self.refresh()
        dlg[0] = MDDialog(title="Nova Categoria", type="custom", content_cls=content,
